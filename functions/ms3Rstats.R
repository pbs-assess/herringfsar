...
.simPerfStats <- function( obj, inclAgg=FALSE, exclNoTAC=FALSE  )
{
  ...
  Bmsy_isp      <- array(NA, dim=c(nGoodReps,nS,nP))
  USR_isp       <- array(NA, dim=c(nGoodReps,nS,nP))
  ...
  for(i in 1:nGoodReps)
  {
    ...
    if(is.null(obj$ctlList$refPts$B0))
      B0_isp[i,,]   <- rp$B0_sp
    else B0_isp[i,,] <- obj$ctlList$refPts$B0
    
    if(is.null(obj$ctlList$refPts$Bmsy))
      Bmsy_isp[i,,]   <- NA
    else Bmsy_isp[i,,] <- obj$ctlList$refPts$Bmsy
  
    
    # Pull out Dstar and Dscalar
    tdxUSR        <- obj$ctlList$opMod$histCtl$hypo$Dstar - 1950
    scalarUSR     <- obj$ctlList$opMod$histCtl$hypo$Dscalar
    # 
  ...
  { 
    ...
    retroSB_it <-  obj$mp$assess$retroSB_itspt[allConvReps,1,1,1,]
    ...
    Uref_isp <- array(obj$ctlList$mp$hcr$Uref_p, dim = c(nGoodReps,nS,nP))
    ...
    endD_ispt <- endSB_ispt
    for(t in 1:nT )
      endD_ispt[,,,t] <- endSB_ispt[,,,t]/B0_isp
    ...
    # Calculate probability that Bt above 50% unfished
    ...
    # Calculate probability that Bt above 60% unfished
    ...
    # Calculate probability that Bt above .4 Bmsy
    pBtGt.4Bmsy_sp <- .calcStatsProportion( TS_ispt = endSB_ispt,
                                            ref_isp = Bmsy_isp,
                                            tdx = tMP:nT,
                                            prop = .4,
                                            nS = nS,
    ...
    # Calculate probability that Bt above 50% unfished
    pBtGt.8Bmsy_sp <- .calcStatsProportion( TS_ispt = endSB_ispt,
                                            ref_isp = Bmsy_isp,
                                            tdx = tMP:nT,
                                            prop = .8,
                                            nS = nS,
    ...
    # Calculate probability that Bt above 60% unfished
    pBtGtBmsy_sp <- .calcStatsProportion(   TS_ispt = endSB_ispt,
                                            ref_isp = Bmsy_isp,
                                            tdx = tMP:nT,
                                            prop = 1,
                                            nS = nS,
    ...
    # Target USR
    pBtGtUSR_sp <- .calcStatsProportion(    TS_ispt = endSB_ispt,
                                            ref_isp = USR_isp,
                                            tdx = tMP:nT,
                                            prop = 1,
                                            nS = nS,
    ...
    pUGtUref_sp <- .calcStatsProportion(    TS_ispt = U_ispt,
                                            ref_isp = Uref_isp,
                                            tdx = tMP:nT,
                                            prop = 1,
                                            nS = nS,
    ...
    B5pc_sp   <- apply( X = endSB_ispt[,,,tMP:nT,drop = FALSE], 
                        FUN = quantile,
                        MARGIN = 2:3, na.rm = T,
                        probs = 0.05 )

    D5pc_sp   <- apply( X = endD_ispt[,,,tMP:nT,drop = FALSE], 
                        FUN = quantile,
                        MARGIN = 2:3, na.rm = T,
                        probs = 0.05 )
    

    C5pc_sp   <- apply( X = C_ispt[,,,tMP:nT,drop = FALSE], 
                        FUN = quantile,
                        MARGIN = 2:3, na.rm = T,
                        probs = 0.05 )

    # # Viable fishery
    pCtGt650_sp <- .calcStatsProportion(  TS_ispt = C_ispt,
                                          ref_isp = array(0.65, dim = c(nGoodReps,nS,nP)),
                                          tdx = tMP:nT,
                                          prop = 1,
                                          nS = nS,
                                          nP = nP )
    ...
    overFishUbar_sp <- apply( X = overFishUbar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
    ...
    sokYrsBar_sp   <- apply( X = sokYrsBar_isp[,,,drop=FALSE], FUN = median, MARGIN = c(2,3))
    ...
    if(exclNoTAC)
    {
      ...
      Cbar_sp     <- apply( X = Cbar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
      Pbar_sp     <- apply( X = Pbar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
      SOKbar_sp   <- apply( X = SOKbar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)

      # SOK licenses
      Lbar_sp   <- apply( X = LBar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
    ...
    if(!exclNoTAC)
    {
      ...
      # catch, live ponded biomass, SOK product in metric tonnes, and SOK licenses
      Cbar_sp     <- apply( X = Cbar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
      Pbar_sp     <- apply( X = Pbar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
      SOKbar_sp   <- apply( X = SOKbar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
      Lbar_sp     <- apply( X = LBar_isp[,,,drop = FALSE], FUN = median, MARGIN = c(2,3), na.rm = T)
    ...
    for( s in 1:nS )
      for(p in 1:nP )
      {
        ...
        statTable[rowIdx,"B5pc"]          <- round(B5pc_sp[s,p],3)
        statTable[rowIdx,"D5pc"]          <- round(D5pc_sp[s,p],3)
        statTable[rowIdx,"pBtGt.5B0"]     <- round(pBtGt.5B0_sp[s,p],3)
        statTable[rowIdx,"pBtGt.6B0"]     <- round(pBtGt.6B0_sp[s,p],3)
        statTable[rowIdx,"pBtGtUSR"]      <- round(pBtGtUSR_sp[s,p],3)
        ...
        statTable[rowIdx,"pCtGt650t"]     <- round(pCtGt650_sp[s,p],2)
        statTable[rowIdx,"C5pc"]          <- round(C5pc_sp[s,p],3)
        # statTable[rowIdx,"pFtGtFmsy"]       <- round(pFtGtFmsy_sp[s,p],2)

        # Catch statistics
        statTable[rowIdx,"pUtGtUref"]     <- round(pUGtUref_sp[s,p],3)
        ...
        statTable[rowIdx,"BtMP"]            <- round(mean(SB_ispt[,s,p,tMP],na.rm = T),3)
        statTable[rowIdx,"BhattMP"]         <- round(mean(retroSB_it[,tMP],na.rm = T),3)
        statTable[rowIdx,"DtMP"]            <- round(mean(endSB_ispt[,s,p,tMP]/B0_isp[,s,p],na.rm = T),3)
        statTable[rowIdx,"CtMP"]            <- round(mean(C_ispt[,s,p,tMP],na.rm = T),3)
    ...
    if(inclAgg)
    {
      ...
      overFishUbar_agg <- median(overFishUbar_i,na.rm=T)
      ...
      if(exclNoTAC)
      {
        ...
        # calculate average stats across projection years for each rep
        ...
        # calculate median across reps
        Cbar_agg    <- median(Cbar_i)
        Pbar_agg    <- median(Pbar_i)
        SOKbar_agg  <- median(SOKbar_i)
        Lbar_agg   <- median(Lbar_i)
      ...
      aggRow[,"CtMP"]            <- round(mean(C_it[,tMP],na.rm = T),3)
