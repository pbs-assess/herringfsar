...
A weighted ensemble of SISCAH operating models is used for management procedure 
evaluation, containing five different models 
representing a range of productivity levels. 
Productivity is characterized by stock recruitment steepness 
...
Details of the operating model ensemble are described in Appendix [A](#sec:app-a).
...
<!-- Mandatory -->
This SISCAH modelling framework [@johnson2024] provides SoG stock status and 
quota calculations for 2024/25 season using the MP tuned to the conservation 
objective.
...
spawning areas, this may provide more insight on sub-stock structure in the Salish Sea.

### Reference Points {-}
<!-- [Mandatory. -->
<!-- A table may be used in lieu of bullets for this section. -->
<!-- Refer to the Guidance Document for further instructions.] -->
For Pacific Herring MSE we present a suite of biological reference 
points as performance metrics for the MP evaluations. 
These include fractions of $B_0$ (unfished spawning biomass) and $B_{MSY}$ (biomass at 
maximum sustainable yield). 
Figure \@ref(fig:yieldSSBcurve) shows relative position of biological reference
...
A biomass target objective centered on the USR has been brought up within the 
...
Currently we do not specify an additional RR for SoG Herring because with 
MSE we are using simulations to select a MP tuned directly to the 
...
fully representing the trophic level of Herring as a forage species.
...
We explore two hockey-stick shapes which differ in their lower and upper
...
For each interim year, stock status 
is estimated using the estimation model component of the MP. 
This estimate plus the new data (survey index and realized catch) are 
compared to the simulation envelopes from the ensemble model.
Exceptional circumstances are determined 
by visually comparing survey index and catch estimates for the new year
...
SoG data are described in full in the 2024 SoG data summary report
(https://github.com/grinnellm/Reports/blob/master/Summaries/2024/DataSummary.SoG.2024.pdf).
