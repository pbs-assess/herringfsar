...
points as performance metrics for the MP evaluations. Reference points are 
fractions of $B_0$ (unfished spawning biomass) and $B_{MSY}$ (biomass at 
maximum sustainable yield).


```{r yieldSSBcurve, echo = FALSE, fig.cap = yieldSSBcap}
yieldSSBcap <- "Strait of Georgia herring operating model 1 equilibrium yield curve with respect to spawning stock biomass. The horizontal dashed line segment indicates OM1's estimate of maximum sustainable yield ($MSY$), while vertical line segments show biomass reference points inluding, from right to left, unfished biomass ($B_0$), the provisional upper stock reference ($USR$), optimal biomass producing $MSY$ on average ($B_{MSY}$), 80\\% of $B_{MSY}$, the current limit reference point ($LRP = 0.3 B_0$), and the default LRP suggested by DFO policy ($0.4 B_{MSY}$)."
...
SDNJ: Sure, we can add that here. I wonder if we should remove reference to $MSY$ 
above, as we don't use any fractions of MSY for MP evaluations. Do we mean $B_{MSY}$?
...
Herring major stocks as $0.3B_0$, where ${B_0}$ is the long-term
...
<!-- Is the above correct for the 5 metrics? Do we use 0.8 Umsy? 
...
1. A LRP of $0.3\cdot {B_0}$ based on an analysis of stock productivity [@kronlund2017]
...
The LRP of $0.3\cdot {B_0}$ for the ensemble operating model is
approximately $0.7 \cdot B_{MSY}$ (Appendix A).
