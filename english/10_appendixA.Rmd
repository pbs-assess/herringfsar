...
based on $U_{MSY}$([@dfo2009],Figure \@ref(fig:hcrPlot), Table \@ref(tab:statTable)).
...
Selecting a MP by tuning to the conservation objective leads to a harvest control rule 
recommendation with a 14\% maximum target harvest rate. If applied annually, the
average yield, using recent 20-year average allocation among gear types,
is 10 - 12 kt (Table \@ref(tab:statTable), Figure \@ref(fig:simEnv)). Selecting
and MP with a lower maximum target harvest rate will also achieve the 
...
further MP tuning. If this was undertaken, for example, tuning the MP
...
the maximum total harvest rate (Figure \@ref(fig:simEnv), bottom row). 
However because we have included this EM in the simulation-testing process, 
any positive assessment errors are accounted for in the MP tuning for meeting
the conservation objective.
<!-- DELETE: "As such, MPs should aim for slightly lower maximum harvest rates than the biomass target implies.-->
<!-- TODO (verify explanation)Sam would have to verify this,
I don't think we used the EM in the simulation-testing process, we used the OM.
Perhaps we could say:
However because realized harvest if often historically below the target rate,
the positive assessment errors are should not interfere with meeting
the conservation objective.
-->
...
SBtRtMtFtCaption <- paste0(
  "Time series of spawning biomass in kilotonnes (kt)",
  " and depletion with scaled spawn indices,",
  " recruitment in millions of fish,",
  " natural mortality, as well as",
  " harvest rate and catch in kt for Strait of Georgia Pacific Herring."
)
