...
statTabCap <- "Performance statistics for MPs differing by maximum target harvest rates. Objectives that are met by an MP are indicated by a checkmark. For all columns, the subscript $t$ indicates the yeras 2024 - 2038. Biomass $B_t$ and Harvest Rate $U_t$ metrics are in probability units. Average Catch $\\overline{C_t}$ and Final Biomass $B_{2038}$ are in biomass units (kt), and final year $B_{2038}/B_0$ and average projection year $\\overline{B_t/B_0}$ stock status is biomass depletion relative to unfished."
