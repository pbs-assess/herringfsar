...
parameter is better estimated when there has been low stock sizes, which 
...
Centrally we use an established value of $h=0.70$ [TODO: add ref] 
...
$h$value. 
We then executed models with $h$ values of 0.65, 0.07, and 0.75, 
...
of 0.34. The remaining operating models are weighted equally across the remaining  
with a weight of 0.165. This creates OM1's weight is the average of the weights 
of each axis in the cross design, with $h$ being uniform (i.e., 0.33) and the 
...
2.  Project the SOG Herring DDM operating model into the future one time-step 
at a time. At each time step t, apply the following:
      ...
      ii)  Using an assessment model (the Density Independent $M$ SISCAH model) 
      ...
      iv)  Calculate the total allowable catch from the 1-year ahead biomass 
      forecast and the determined target harvest rate;
      v)  Update the simulated DDM operating model population given the total 
      allowable catch from the MP and incoming recruitment simulated off the the 
      DDM operating model's stock-recruit curve with recruitment process errors;
