...
at a time. For each year in the projection, apply the following:
      ...
      v)  Update the simulated DDM operating model Herring population the with 
...
# Management during the interim years {-}

Once a harvest control rule is chosen through the MSE 
process that harvest control rule defines the maximum target harvest rate 
until the next full MSE cycle. 
For Pacific Herring in BC, the recommendation is a minimum 3 year cycle [@dfo2023a]; 
the MSE will be updated for SoG in 2026.
In the intervening years an estimate of biomass is obtained using the estimation
model within the selected MP (i.e., the less computationally intensive DIM SISCAH model).
Note that quota calculations are only presented for the MP tuned to the 
conservation objective.

# Discussion {-}

A fishery that meets Objective 1 could target a 14\% harvest rate with an 
average yield in the 10 - 12 kt range (Table \@ref(tab:statTable),
Figure \@ref(fig:simEnv)).

If management procedures are also targeting the USR (Objective 2), then harvest rates need
